...
!  .. LOCAL ARRAYS ..
   ...
   REAL(WP), ALLOCATABLE :: A(:,:), B(:,:), AF(:,:), X(:,:), B1(:), X1(:)
...
!
   WRITE ( NOUT, * )'---------------------------------------------------------'
   WRITE ( NOUT, * )
   WRITE ( NOUT, * )'Details of LA_SGESVX LAPACK SUBROUTINE Results.'
   WRITE ( NOUT, * )
!
   WRITE(NOUT,*)
...
! 
   ...
   WRITE (NOUT,FMT) X1(:)
!
END PROGRAM LA_SGESVX_ET_EXAMPLE
