...
*     .. External Subroutines ..
      EXTERNAL           LA_TEST_SGEEVX, SGET22, SLACPY, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IINFO = 0
      CALL LA_TEST_SGEEVX( BALANC, 'V', 'V', SENSE, N, H, LDA, WR,
     &             WI, VL, LDVL,
...
*
         CALL SLACPY( 'F', N, N, A, LDA, H, LDA )
         IINFO = 0
         CALL LA_TEST_SGEEVX( BALANC, 'N', 'N', SENSE, N, H, LDA,
     &                WR1, WI1, DUM,
...
*
         CALL SLACPY( 'F', N, N, A, LDA, H, LDA )
         IINFO = 0
         CALL LA_TEST_SGEEVX( BALANC, 'N', 'V', SENSE, N, H, LDA, WR1,
     &                WI1, DUM,
...
*
         CALL SLACPY( 'F', N, N, A, LDA, H, LDA )
         IINFO = 0
         CALL LA_TEST_SGEEVX( BALANC, 'V', 'N', SENSE, N, H, LDA, WR1,
     &                WI1, LRE,
...
*
      IF( COMP ) THEN
        CALL SLACPY( 'F', N, N, A, LDA, H, LDA )
        IINFO = 0
         CALL LA_TEST_SGEEVX( 'N', 'V', 'V', 'B', N, H, LDA, WR, WI,
     &                VL, LDVL,
     $                VR, LDVR, ILO, IHI, SCALE, ABNRM, RCONDE, RCONDV,
     $                WORK, LWORK, IWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
