SUBROUTINE LA_TEST_DGERFS( TRANS, N, NRHS, A, LDA, AF, LDAF, IPIV, B, LDB, X, LDX, FERR, BERR, WORK, IWORK, INFO )
!
...
!  .. Array Arguments ..
   INTEGER, INTENT(INOUT) :: IPIV(1:N), IWORK(*)
   REAL(WP), INTENT(INOUT) :: A(1:LDA,1:N), AF(1:LDAF,1:N), B(1:LDB,1:NRHS), &
             BERR(1:NRHS), FERR(1:NRHS), WORK( * ), X(1:LDX,1:NRHS)
!  .. Parameters ..
   CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GERFS'
   CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_DGERFS'
!  .. Common blocks ..
   INTEGER :: INFOTC
...
!               B(1:IB1,1:IB2),  X(1:IX1,1:IX2), FERR=FERR(1:IFERR), &
!               BERR=BERR(1:IBERR) )
!         ELSE
!           CALL LA_GERFS(A(1:IA1,1:IA2), AF(1:IAF1,1:IAF2), IPIV(1:IIPIV), &
...
!         IF( LTRANS == 'N' )THEN
!           CALL LA_GERFS(A(1:IA1,1:IA2), AF(1:IAF1,1:IAF2), IPIV(1:IIPIV), &
!               B(1:IB1,1),  X(1:IX1,1), FERR=FERR(1), BERR=BERR(1) )
!         ELSE
!           CALL LA_GERFS(A(1:IA1,1:IA2), AF(1:IAF1,1:IAF2), IPIV(1:IIPIV), &
!               B(1:IB1,1),  X(1:IX1,1), LTRANS, FERR(1), BERR(1) )
!         END IF
!         INFO = INFOTC
!      END SELECT J0
   CASE (1)
