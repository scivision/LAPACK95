SUBROUTINE LA_TEST_SSYSVX( FACT, UPLO, N, NRHS, A, LDA, AF, LDAF, IPIV, &
     &  B, LDB, X, LDX, RCOND, FERR, BERR, WORK, LWORK, IWORK, INFO )
!
...
!  .. Array Arguments ..
      ...
      INTEGER :: IWORK(1: N)
      REAL(WP), INTENT(INOUT) :: AF(1:LDAF,1:N)
      REAL(WP), INTENT(OUT) :: X(1:LDX, 1:NRHS), &
     &  FERR(1: NRHS), BERR(1: NRHS)
      REAL(WP), INTENT(IN) :: A(1:LDA,1:N), B(1:LDB,1:NRHS), &
     &  WORK(1:LWORK)
!  .. Parameters ..
      CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_SYSVX'
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_SSYSVX'
!  .. Common blocks ..
      INTEGER :: INFOTC
