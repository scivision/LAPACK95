SUBROUTINE LA_TEST_CGELSX( M, N, NRHS, A, LDA, B, LDB, JPVT, RCOND, RANK, WORK, RWORK, INFO )
!
...
!  .. Array Arguments ..
   ...
   COMPLEX(WP), INTENT(OUT) :: WORK(1: MAX(MIN(M,N)+3*N, &
&                                   2*MIN(M,N)+NRHS*NRHS))
   INTEGER, INTENT(INOUT) :: JPVT(1:N)
   REAL(WP) :: RWORK(1: 2*N)
!  .. Parameters ..
   CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GELSX'
   CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_CGELSX'
!  .. Common blocks ..
   INTEGER :: INFOTC
