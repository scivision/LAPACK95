SUBROUTINE LA_TEST_CGESVX( FACT, TRANS, N, NRHS, A, LDA, AF, LDAF, IPIV, &
...
!  .. Array Arguments ..
   ...
   COMPLEX(WP), INTENT(INOUT) :: A(1:LDA,1:N), AF(1:LDAF,1:N), B(1:LDB,1:NRHS), &
                                 WORK(2*N), X(1:LDX,1:NRHS)
   REAL(WP), INTENT(INOUT) :: BERR(1:NRHS), FERR(1:NRHS), R(1:N), C(1:N), &
                             RWORK(2*N)
!  .. Parameters ..
   CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GESVX'
   CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_CGESVX'
!  .. Common blocks ..
   INTEGER :: INFOTC
