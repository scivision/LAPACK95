SUBROUTINE LA_TEST_ZGELSX( M, N, NRHS, A, LDA, B, LDB, JPVT, RCOND, RANK, WORK, RWORK, INFO )
!
...
!  .. Array Arguments ..
   ...
   COMPLEX(WP), INTENT(OUT) :: WORK(1:3*MIN(M,N)+ MAX(N, 2*MIN(M,N)+NRHS))
   INTEGER, INTENT(INOUT) :: JPVT(1:N)
   REAL(WP) :: RWORK(1: 2*N)
!  .. Parameters ..
   CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GELSX'
   CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_ZGELSX'
!  .. Common blocks ..
   INTEGER :: INFOTC
