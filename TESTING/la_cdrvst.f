...
*     .. External Subroutines ..
      EXTERNAL           ALASVM, LA_TEST_CHBEV, LA_TEST_CHBEVD,
     &                   LA_TEST_CHBEVX,  LA_TEST_CHEEV, LA_TEST_CHEEVD,
     $                   LA_TEST_CHEEVR, CHEEVX, CHET21, CHET22,
     &                   LA_TEST_CHPEV, LA_TEST_CHPEVD, LA_TEST_CHPEVX,
     &                   CLACPY, CLASET, CLATMR, CLATMS, SLABAD,
     $                   SLAFTS, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               NTEST = NTEST + 1
               IINFO = 0
               CALL LA_TEST_CHEEVD( 'V', UPLO, N, A, LDU, D1, WORK,
     &           LWEDC,
...
*
               NTEST = NTEST + 2
               IINFO = 0
               CALL LA_TEST_CHEEVD( 'N', UPLO, N, A, LDU, D3, WORK,
     &           LWEDC,
     $                      RWORK, LRWEDC, IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0
               CALL CHEEVX( 'V', 'A', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
...
*
               NTEST = NTEST + 2
               IINFO = 0
               CALL CHEEVX( 'N', 'A', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
...
*
               IINFO = 0
               CALL CHEEVX( 'V', 'I', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, WORK, LWORK, RWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0
               CALL CHEEVX( 'N', 'I', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
...
*
               IINFO = 0
               CALL CHEEVX( 'V', 'V', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, WORK, LWORK, RWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0 
               CALL CHEEVX( 'N', 'V', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M3, WA3, Z, LDU, WORK, LWORK, RWORK,
     $                      IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               ...
               INDWRK = N*( N+1 ) / 2 + 1
               IINFO = 0
               CALL LA_TEST_CHPEVD( 'V', UPLO, N, WORK, D1, Z, LDU,
...
*
               ...
               INDWRK = N*( N+1 ) / 2 + 1
               IINFO = 0
               CALL LA_TEST_CHPEVD( 'N', UPLO, N, WORK, D3, Z, LDU,
     $                      WORK( INDWRK ), LWEDC, RWORK, LRWEDC, IWORK,
     $                      LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'V', 'A', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'N', 'A', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'V', 'I', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, V, RWORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'N', 'I', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'V', 'V', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, V, RWORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               IINFO = 0
               CALL LA_TEST_CHPEVX( 'N', 'V', UPLO, N, WORK, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M3, WA3, Z, LDU, V, RWORK, IWORK,
     $                      IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 1
               IINFO = 0
               CALL LA_TEST_CHBEVD( 'V', UPLO, N, KD, V, LDU, D1, Z,
     &           LDU, WORK,
...
*
               NTEST = NTEST + 2
              IINFO = 0
               CALL LA_TEST_CHBEVD( 'N', UPLO, N, KD, V, LDU, D3, Z,
     &           LDU, WORK,
     $                      LWEDC, RWORK, LRWEDC, IWORK, LIWEDC, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 1
               IINFO = 0
               CALL LA_TEST_CHBEVX( 'V', 'A', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
...
*
               IINFO = 0
               CALL LA_TEST_CHBEVX( 'N', 'A', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
...
*
               IINFO = 0
               CALL LA_TEST_CHBEVX( 'V', 'I', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
     $                      VU, IL, IU, ABSTOL, M2, WA2, Z, LDU, WORK,
     $                      RWORK, IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
  ...
  830             CONTINUE
                END IF
                IINFO = 0
               CALL LA_TEST_CHBEVX( 'N', 'I', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
...
*
  ...
  880             CONTINUE
                END IF
                IINFO = 0
               CALL LA_TEST_CHBEVX( 'V', 'V', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
     $                      VU, IL, IU, ABSTOL, M2, WA2, Z, LDU, WORK,
     $                      RWORK, IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
  ...
  920             CONTINUE
                END IF
                INFO = 0
               CALL LA_TEST_CHBEVX( 'N', 'V', UPLO, N, KD, V, LDU, U,
     &                      LDU, VL,
     $                      VU, IL, IU, ABSTOL, M3, WA3, Z, LDU, WORK,
     $                      RWORK, IWORK, IWORK( 5*N+1 ), IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 1
              IINFO = 0
               CALL LA_TEST_CHEEV( 'V', UPLO, N, A, LDU, D1, WORK,
     &           LWORK, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 2
              IINFO = 0
               CALL LA_TEST_CHEEV( 'N', UPLO, N, A, LDU, D3, WORK,
     &           LWORK, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               ...
               INDWRK = N*( N+1 ) / 2 + 1
              IINFO = 0
               CALL LA_TEST_CHPEV( 'V', UPLO, N, WORK, D1, Z, LDU,
...
*
               ...
               INDWRK = N*( N+1 ) / 2 + 1
              IINFO = 0
               CALL LA_TEST_CHPEV( 'N', UPLO, N, WORK, D3, Z, LDU,
     $                     WORK( INDWRK ), RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 1
               IINFO = 0
               CALL LA_TEST_CHBEV( 'V', UPLO, N, KD, V, LDU, D1, Z,
     &           LDU, WORK,
     $                     RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 2
              IINFO = 0
               CALL LA_TEST_CHBEV( 'N', UPLO, N, KD, V, LDU, D3, Z,
     &           LDU, WORK,
     $                     RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               ...
               NTEST = NTEST + 1
               IINFO = 0
               CALL LA_TEST_CHEEVR( 'V', 'A', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M, WA1, Z, LDU, IWORK, WORK, LWORK,
     $                      RWORK, LRWORK, IWORK( 2*N+1 ), LIWORK-2*N,
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'CHEEV(V,A,' // UPLO //
     $               ')', IINFO, N, JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = NTEST + 2
               IINFO = 0
               CALL LA_TEST_CHEEVR( 'N', 'A', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
...
*
               ...
               CALL CLACPY( ' ', N, N, V, LDU, A, LDA )
               IINFO = 0
               CALL LA_TEST_CHEEVR( 'V', 'I', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, IWORK, WORK, LWORK,
     $                      RWORK, LRWORK, IWORK( 2*N+1 ), LIWORK-2*N,
     $                      IINFO )
...
*
               ...
               CALL CLACPY( ' ', N, N, V, LDU, A, LDA )
               IINFO =0 
               CALL LA_TEST_CHEEVR( 'N', 'I', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
...
*
               ...
               CALL CLACPY( ' ', N, N, V, LDU, A, LDA )
               IINFO = 0
               CALL LA_TEST_CHEEVR( 'V', 'V', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M2, WA2, Z, LDU, IWORK, WORK, LWORK,
     $                      RWORK, LRWORK, IWORK( 2*N+1 ), LIWORK-2*N,
     $                      IINFO )
...
*
               ...
               CALL CLACPY( ' ', N, N, V, LDU, A, LDA )
               IINFO =0 
               CALL LA_TEST_CHEEVR( 'N', 'V', UPLO, N, A, LDU, VL, VU,
     &                      IL, IU,
     $                      ABSTOL, M3, WA3, Z, LDU, IWORK, WORK, LWORK,
     $                      RWORK, LRWORK, IWORK( 2*N+1 ), LIWORK-2*N,
     $                      IINFO )
