SUBROUTINE LA_TEST_SGESVX( FACT, TRANS, N, NRHS, A, LDA, AF, LDAF, IPIV, &
...
!  .. Array Arguments ..
   ...
   REAL(WP), INTENT(INOUT) :: A(1:LDA,1:N), AF(1:LDAF,1:N), B(1:LDB,1:N), &
                              BERR(1:NRHS), FERR(1:NRHS), WORK(*), &
                              X(1:LDX,1:NRHS), R(1:N), C(1:N)
!  .. Parameters ..
   CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GESVX'
   CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_SGESVX'
!  .. Common blocks ..
   INTEGER :: INFOTC
...
!  .. Executable Statements ..
   ...
   CASE(0)
      IF (NRHS == 1) THEN
        ...
        R(1:IR), C(1:IC), FERR(1), BERR(1), RCOND, WORK(1) )
        INFO = INFOTC
      ELSE
   ...
   IF( .NOT. ( ( I == 6 .AND. ( J == 2 .OR. J == 3 .OR. J == 4 ) ) &
      ...
      CASE (1)
         IF( INFO /= 202 .AND. INFO /= 302 .AND. I /= 11 .AND. I /= 12 )THEN
            CALL LA_GESVX(A(1:IA1,1:IA2), B(1:IB1,1), X(1:IX1,1), &
                 AF(1:IAF1,1:IAF2), IPIV(1:IIPIV), LFACT, LTRANS, LEQUED, &
                 R(1:IR), C(1:IC), FERR(1), BERR(1), RCOND, WORK(1), INFO)
          ELSE
