SUBROUTINE LA_TEST_DSPGVX(ITYPE, JOBZ, RANGE, UPLO, N, AP, BP,&
...
!  .. Array Arguments ..
      ...
      REAL(WP), INTENT(INOUT) :: WORK(1:8*N), AP(1:N*(N+1)/2), BP(1:N*(N+1)/2)
      REAL(WP), INTENT(OUT) :: W(1:N),  Z(1:LDZ,1:MAX(1,N))
      INTEGER, INTENT(OUT) :: IFAIL(1:N)
      LOGICAL LSAME
!  .. Parameters ..
      CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_SPGVX'
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_DSPGVX'
!  .. Common blocks ..
      INTEGER :: INFOTC
...
!  .. Executable Statements ..
      ...
      IVU = IVL -1
      CASE (8)
      CALL LA_SPGVX( AP(1:IAP), BP(1:IBP), W=W, ITYPE=IITYPE, &
     ...
     &  ABSTOL=IABSTOL, INFO=INFO )
      ...
      IF (LSAME(IJOBZ, 'V')) THEN
        IF (LSAME(IRANGE,'A')) THEN
          CALL LA_SPGVX( AP(1:IAP), BP(1:IBP), W, IITYPE,  IUPLO,&
