SUBROUTINE LA_TEST_CPTSVX( FACT, N, NRHS, D, E, DF, EF, B, LDB, X, LDX,&
...
!  .. Array Arguments ..
      REAL(WP), INTENT(IN) :: D(1:N)
      COMPLEX(WP), INTENT(IN) :: E(1:N-1), B(1:LDB,1:NRHS)
      REAL(WP), INTENT(INOUT) :: DF(1:N)
      COMPLEX(WP), INTENT(INOUT) :: EF(1:N)
      COMPLEX(WP), INTENT(OUT) :: X(1:LDX,1:NRHS), WORK(1:N)
      REAL(WP), INTENT(OUT) :: FERR(1:NRHS), BERR(1:NRHS)
      REAL(WP), INTENT(OUT) :: RWORK(1: N)
!  .. Parameters ..
      CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_PTSVX'
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_CPTSVX'
!  .. Common blocks ..
      INTEGER :: INFOTC
