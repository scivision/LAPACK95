SUBROUTINE LA_TEST_CHEEVX( JOBZ, RANGE, UPLO,  N, A, LDA, VL, VU, IL, &
...
!  .. Parameters ..
      ...
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_CHEEVX'
      LOGICAL LSAME
      EXTERNAL LSAME
!  .. Common blocks ..
      INTEGER :: INFOTC
...
!  .. Executable Statements ..
      ...
      CASE (3)
        IJOBZ = 'T'
        CALL LA_HEEVX( A(1:IA1,1:IA2), W(1 :IW), IJOBZ, IUPLO, &
...
&         ABSTOL=ABSTOL, INFO=INFO )
        ...
        RETURN
      CASE (8)
        IIU = IA2 + 1;  IIL = IIU; IJOBZ='V'; IRANGE='I'     
        CALL LA_HEEVX( A(1:IA1,1:IA2), W(1 :IW), IJOBZ, IUPLO, &
&         IL=IIL, IU=IIU, M=M, ABSTOL=ABSTOL, INFO=INFO )
        CALL LA_AUX_AA01( I, CTEST, ETEST, SRNAMT )
        RETURN
      CASE(10)
        IIFAIL = IA2 - 1; IJOBZ='V'; IRANGE='A'
        CALL LA_HEEVX( A(1:IA1,1:IA2), W(1 :IW), IJOBZ, IUPLO, &
&         M=M, IFAIL=IFAIL(1:IIFAIL), ABSTOL=ABSTOL, INFO=INFO )
        CALL LA_AUX_AA01( I, CTEST, ETEST, SRNAMT )
        RETURN
...
&         M=M, IFAIL=IFAIL(1:IIFAIL), ABSTOL=ABSTOL, INFO=INFO )
      ELSE
        CALL LA_HEEVX( A(1:IA1,1:IA2), W(1 :IW), UPLO=IUPLO, &
