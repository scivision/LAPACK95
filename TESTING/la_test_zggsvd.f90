      SUBROUTINE LA_TEST_ZGGSVD(JOBU, JOBV, JOBQ, M, N, P, K, L, A, LDA, B, &
     &  LDB, ALPHA, BETA, U, LDU, V, LDV, Q, LDQ, WORK, RWORK, IWORK, INFO)
!
...
!  .. Array Arguments ..
      ...
      COMPLEX(WP), INTENT(OUT):: WORK(1: MAX(3*N, M, P)+N)
      REAL(WP), INTENT(OUT) :: ALPHA(1: N), BETA(1: N)
      COMPLEX(WP), INTENT(OUT) ::  V(1: LDV, 1:P), Q(1: LDQ, 1: N), &
     &  U(1: LDU, 1:M)
      REAL(WP) :: RWORK(1: 2*N)
      INTEGER, INTENT(OUT) :: IWORK(1: N)
!  .. Parameters ..
      CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_GGSVD'
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_ZGGSVD'
!  .. Common blocks ..
      INTEGER :: INFOTC
...
&             V=V(1: IV1, 1: IV2), Q=Q(1: IQ1, 1: IQ2), IWORK=IWORK, &
&             INFO=INFO)
          ELSE
            CALL LA_GGSVD( A(1:IA1,1:IA2), B(1: IB1, 1:IB2), &
     &        ALPHA(1: IALPHA), BETA(1: IBETA), K, L, &
