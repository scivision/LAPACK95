SUBROUTINE LA_TEST_SSBGVX(JOBZ, RANGE, UPLO, N, KAB, KBB, AB, LDAB, BB,&
...
!  .. Array Arguments ..
      ...
      REAL(WP), INTENT(OUT) :: WORK(1:7*N), Q(1:LDQ,1:N), W(1:N), &
     &  Z(1:LDZ,1:N)
      INTEGER, INTENT(OUT) :: IFAIL(1:N)
      LOGICAL LSAME
!  .. Parameters ..
      CHARACTER(LEN=8),  PARAMETER :: SRNAME = 'LA_SBGVX'
      CHARACTER(LEN=14), PARAMETER :: SRNAMT = 'LA_TEST_SSBGVX'
!  .. Common blocks ..
      INTEGER :: INFOTC
...
!  .. Executable Statements ..
      ...
      IVU = IVL -1
      CASE (7)
      CALL LA_SBGVX( AB(1:IAB1,1:IAB2), BB(1:IBB1,1:IBB2), W=W, &
     ...
     &  Q=Q(1:IQ1,1:IQ2), ABSTOL=IABSTOL, INFO=INFO )
      ...
      IF (LSAME(IJOBZ, 'V')) THEN
        IF (LSAME(IRANGE,'A')) THEN
          CALL LA_SBGVX( AB(1:IAB1,1:IAB2), BB(1:IBB1,1:IBB2), W, IUPLO,&
