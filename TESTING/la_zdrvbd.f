...
*     .. External Subroutines ..
      EXTERNAL           ALASVM, XERBLA, ZBDT01, LA_TEST_ZGESDD,
     &                   LA_TEST_ZGESVD,
...
*
     ...
     $            CALL ZLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               IINFO = 0
               CALL LA_TEST_ZGESVD( 'A', 'A', M, N, A, LDA, SSAV,
     &           USAV, LDU, VTSAV, LDVT, WORK, LSWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 90
                     ...
                     CALL ZLACPY( 'F', M, N, ASAV, LDA, A, LDA )
                     IINFO = 0
                     CALL LA_TEST_ZGESVD( JOBU, JOBVT, M, N, A, LDA,
     $                 S, U, LDU,
...
*
               CALL ZLACPY( 'F', M, N, ASAV, LDA, A, LDA )
               IINFO = 0
               CALL LA_TEST_ZGESDD( 'A', M, N, A, LDA, SSAV, USAV,
     $           LDU, VTSAV, LDVT, WORK, LSWORK, RWORK, IWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               ...
               DO 130 IJQ = 0, 2
                  ...
                  CALL ZLACPY( 'F', M, N, ASAV, LDA, A, LDA )
                  IINFO = 0
                  CALL LA_TEST_ZGESDD( JOBQ, M, N, A, LDA, S, U, LDU,
     $              VT, LDVT, WORK, LSWORK, RWORK, IWORK, IINFO )
